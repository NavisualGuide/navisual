...
/// Is our overlay window sitting BELOW some other app's window at the pointer's rect?
///
/// The periodic re-raise (see the `overlay-raise` thread in `track.rs`) must fire ONLY when the
/// overlay is actually occluded — re-asserting TOPMOST unconditionally every tick churns the
/// z-order of a window that spans the whole virtual desktop, and Windows reveals an auto-hide
/// taskbar whenever a desktop-spanning topmost window is repeatedly re-inserted above it (live
/// 2026-07-24: the pointer-on-top fix made the auto-hide taskbar pop up for the whole guided
/// session). Gating the toggle on real occlusion means that once the overlay is on top, the loop
/// does nothing — no churn, taskbar stays hidden — and it only acts the moment a modal actually
/// appears above the pointer.
///
/// EnumWindows enumerates front→back, so any visible, non-own window seen BEFORE we reach the
/// overlay is above it in z-order. If such a window's frame intersects `pointer_rect`, the pointer
/// is drawn behind it → occluded. Same-PID (our panel/overlay) never counts. The target app's own
/// modal dialog is a different process than navisual-backend, so it correctly counts as an occluder.
pub fn overlay_occluded_in_rect(x: i32, y: i32, w: i32, h: i32) -> bool {
    ...
    struct State {
        our_pid: u32,
        pointer: Rect,
        reached_overlay: bool,
        occluded: bool,
    }

    unsafe extern "system" fn cb(hwnd: HWND, lparam: LPARAM) -> windows::core::BOOL {
        let st = &mut *(lparam.0 as *mut State);
        if st.reached_overlay {
            return FALSE; // done — nothing below the overlay matters
        }
        let ex_style = GetWindowLongW(hwnd, GWL_EXSTYLE) as u32;
        let mut pid: u32 = 0;
        GetWindowThreadProcessId(hwnd, Some(&mut pid));
        // The overlay is our own click-through (WS_EX_TRANSPARENT) window. Reaching it means every
        // window still to come is below it — stop.
        if pid == st.our_pid && (ex_style & WS_EX_TRANSPARENT.0) != 0 && IsWindowVisible(hwnd).as_bool()
        {
            st.reached_overlay = true;
            return FALSE;
        }
        if pid == st.our_pid {
            return TRUE; // our own panel — never an occluder
        }
        if !IsWindowVisible(hwnd).as_bool() || IsIconic(hwnd).as_bool() {
            return TRUE;
        }
        if let Some(fr) = frame_rect_of(hwnd) {
            let hit = rect_intersect(&fr, &st.pointer);
            if hit.width > 0 && hit.height > 0 {
                st.occluded = true;
                st.reached_overlay = true; // decided — stop enumerating
                return FALSE;
            }
        }
        TRUE
