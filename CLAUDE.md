...
└────────────────────────────────────────────────────────┘
                          ▼
┌─ OUTPUT LAYER ─────────────────────────────────────────┐
│ • Overlay renderer (pointers, highlights, subtitles)    │
│ • TTS engine (v0.2)                                     │
...
6. Element Locator finds target_text on live screen (OCR/A11y)
   ↓
7. Render overlay (pointer/highlight/subtitle at exact bbox)
   ↓
...
- **A1 — Native OCR resolution** — OCR now receives a lossless PNG at native screen resolution instead of the downscaled+JPEG AI image. Fixes silent misses on apps with mid-size compact text (VS Code SCM panel confirmed). AI image path unchanged; coordinate translation simplified to crop-origin add only.
- **G2 — Word-boundary substring match** — Replaced `MIN_SUBSTR_LEN=8` with `\b<target>\b` regex in both match directions. Short labels ("OK", "No", "Save") now match; "Insert" no longer matches "InsertedText". Minimum 2 chars enforced.
- **D2 — 3-tier fuzzy cascade** — OCR fuzzy threshold relaxes across three passes (0.85 → 0.75 → 0.65) using already-computed scores (no re-capture). Tier reached is recorded in the debug trace. Tier 3 ignores role preference to maximise recall on difficult targets.
- **C5 — WindowFromPoint hit-test** — After OCR finds a winner, `WindowFromPoint` at the bbox centre checks the leaf HWND class. Denylist: `Static`, `SysHeader32`, `ScrollBar`, `msctls_statusbar32`. Web-renderer classes (`Chrome_RenderWidgetHostHWND`, `MozillaWindowClass`) are exempted. Rejection shown in debug drawer with leaf class name.
- **D6 — Pointer-unavailable indicator** — When the locator returns `None`, a muted one-liner "⊘ Pointer unavailable — follow the instruction above" appears below the instruction. No user action required.
...
v0.6 — Complex Apps + Nav-Packs:
  ...
  5. Grid accuracy benchmark: 3-condition test (plain / lines-only / labeled 16×9
     grid) on ≥20 frozen-PNG fixtures. Determines whether the SoM grid overlay
     improves AI zone accuracy enough to enable by default for paid models.
     See SDD §7.2 grid section for full benchmark spec.
...
v0.6  Template matching + Nav-Packs v1 + Blender/SolidWorks + quantized local models
      + grid accuracy benchmark (SoM grid on/off/labeled, ≥20 fixtures)
...
| Grid zone accuracy (untested) | Medium | SoM grid is implemented but never benchmarked — unclear whether it improves or hurts AI spatial reasoning. Benchmark planned for v0.6. |
